...
public:
    ...
    virtual wchar_t *GetDestValue() const { return nullptr; }
    ...
    // caller must free() the result
    virtual wchar_t *GetDestName() const { return nullptr; }
...
public:
    ...
    explicit DocTocItem(wchar_t *title, int pageNo=0) :
...
public:
    ...
    virtual bool SaveFileAs(const wchar_t *copyFileName, bool includeUserAnnots=false) = 0;
    ...
    virtual bool SaveFileAsPDF(const wchar_t *pdfFileName, bool includeUserAnnots=false) {
        UNUSED(pdfFileName);
    ...
    virtual wchar_t *GetProperty(DocumentProperty prop) = 0;
    ...
    virtual const wchar_t *GetDefaultFileExt() const = 0;
    ...
    // caller must free() the result
    virtual wchar_t *GetPageLabel(int pageNo) const { return str::Format(L"%d", pageNo); }
    // reverts GetPageLabel by returning the first page number having the given label
    virtual int GetPageByLabel(const wchar_t *label) const { return _wtoi(label); }
...
class PasswordUI {
public:
    virtual wchar_t * GetPassword(const wchar_t *fileName, unsigned char *fileDigest,
