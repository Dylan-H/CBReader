...
// Note: the float length of MobiHeader is in MobiHeader.hdrLen. This is just
...
MobiDoc::MobiDoc(const wchar_t *filePath) :
...
wchar_t *MobiDoc::GetProperty(DocumentProperty prop)
{
    for (size_t i = 0; i < props.Count(); i++) {
...
bool MobiDoc::HasToc()
{
    ...
    while ((tok = parser.Next()) != nullptr && !tok->IsError()) {
        ...
        if (!attr)
            continue;
        ScopedMem<wchar_t> val(str::conv::FromHtmlUtf8(attr->val, attr->valLen));
...
bool MobiDoc::ParseToc(EbookTocVisitor *visitor)
{
    ...
    ScopedMem<wchar_t> itemText;
    ScopedMem<wchar_t> itemLink;
    int itemLevel = 0;
    ...
    while ((tok = parser.Next()) != nullptr && !tok->IsError()) {
        if (itemLink && tok->IsText()) {
            ScopedMem<wchar_t> linkText(str::conv::FromHtmlUtf8(tok->s, tok->sLen));
            if (itemText)
...
bool MobiDoc::IsSupportedFile(const wchar_t *fileName, bool sniff)
{
    if (sniff) {
...
MobiDoc *MobiDoc::CreateFromFile(const wchar_t *fileName)
{
