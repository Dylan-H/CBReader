...
struct DrawInstr {
	...
	struct {
        ...
        CanvasTextFormat^ font;        // InstrSetFont
        ImageData       img;          // InstrImage
    };
    Rect bbox; // common to most instructions

    DrawInstr() { }

    explicit DrawInstr(DrawInstrType t, Rect bbox = Rect()) : type(t), bbox(bbox) { }

    // helper constructors for instructions that need additional arguments
    static DrawInstr Str(const char *s, size_t len, Rect bbox, bool rtl=false);
    static DrawInstr Image(char *data, size_t len, Rect bbox);
    static DrawInstr SetFont(CanvasTextFormat^ font);
    static DrawInstr FixedSpace(float dx);
    static DrawInstr LinkStart(const char *s, size_t len);
    static DrawInstr Anchor(const char *s, size_t len, Rect bbox);
};
...
public:
    ...
    void SetFontName(const wchar_t *s) {
        fontName.Set(str::Dup(s));
    }

    const wchar_t *GetFontName() { return fontName; }
...
protected:
    ...
    CanvasTextFormat^  CurrFont() { return CurrStyle()->font; }
    void  SetFont(const wchar_t *fontName, float fontSize=-1);
    void  SetFontBasedOn(CanvasTextFormat^ origFont, float fontSize=-1);
    ...
    float               lineSpacing;
    float               spaceDx;
    CanvasDrawingSession^         gfx; // for measuring text
    ScopedMem<wchar_t>    defaultFontName;
...
public:
    explicit HtmlFormatter(CanvasDrawingSession^ g, HtmlFormatterArgs *args);
...
void DrawHtmlPage(CanvasDrawingSession^ g, ITextRender *textRender, Vec<DrawInstr> *drawInstructions, float offX, float offY, bool showBbox, Windows::UI::Color textColor, bool *abortCookie=nullptr);
