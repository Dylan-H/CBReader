...
class MobiDoc
{
    ...
    explicit MobiDoc(const wchar_t *filePath);
...
public:
    ...
    wchar_t *             GetProperty(DocumentProperty prop);
    PdbDocType          GetDocType() const { return docType; }

    bool                HasToc();
    bool                ParseToc(EbookTocVisitor *visitor);

    static bool         IsSupportedFile(const wchar_t *fileName, bool sniff=false);
    static MobiDoc *    CreateFromFile(const wchar_t *fileName);
};
