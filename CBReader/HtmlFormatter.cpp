...
DrawInstr DrawInstr::Str(const char *s, size_t len, Rect bbox, bool rtl)
{
...
DrawInstr DrawInstr::SetFont(CanvasTextFormat^ font)
{
...
DrawInstr DrawInstr::Image(char *data, size_t len, Rect bbox)
{
...
DrawInstr DrawInstr::Anchor(const char *s, size_t len, Rect bbox)
{
...
HtmlFormatterArgs::HtmlFormatterArgs() :
    ...
    reparseIdx(0), textRenderMethod(TextRenderMethodWin2d_)
{
}

HtmlFormatter::HtmlFormatter(CanvasDrawingSession^ g, HtmlFormatterArgs *args):
    ...
    keepTagNesting(false)
{
    ...
    DrawStyle style;
    style.font = ref new CanvasTextFormat();
    ...
    spaceDx = CurrFont()->FontSize / 2.5f; // note: a heuristic
...
void HtmlFormatter::SetFont(const wchar_t *fontName, float fontSize)
{
    if (fontSize < 0) {
        fontSize = CurrFont()->FontSize;
    }
	CanvasTextFormat^ newfont = ref new CanvasTextFormat();
	newfont->FontSize = fontSize;
	newfont->FontFamily = ref new Platform::String(fontName);
    if (CurrFont() != newfont) {
...
void HtmlFormatter::SetFontBasedOn(CanvasTextFormat^ fs, float fontSize)
{
	/*const wchar_t *fontName = font->GetName();
	if (nullptr == fontName)
...
bool HtmlFormatter::EmitImage(ImageData *img)
{
    ...
    //Size newSize((float)imgSize.Width, (float)imgSize.Height);
    ...
    //Rect bbox(Point(currX, 0), newSize);
...
void HtmlFormatter::EmitHr()
{
    ...
    Rect bbox(0.f, 0.f, pageDx, lineSpacing);
...
void HtmlFormatter::EmitTextRun(const char *s, const char *end)
{
    ...
    while (s < end) {
        ...
        if (0 == strLen)
            break;
        textMeasure->SetFont(CurrFont());
        Rect bbox = textMeasure->Measure(buf, strLen);
        EnsureDx(bbox.Width);
        ...
        // wchar_t doesn't always equal one char
...
void HtmlFormatter::HandleTagP(HtmlToken *t, bool isDiv)
{
    if (!t->IsEndTag()) {
        ...
        if (rule.textIndentUnit != StyleRule::inherit && rule.textIndent > 0) {
            float factor = rule.textIndentUnit == StyleRule::em ? CurrFont()->FontSize :
...
void HtmlFormatter::HandleTagFont(HtmlToken *t)
{
	...
	Platform::String^ text = CurrFont()->FontFamily;
    ...
    float fontSize = CurrFont()->FontSize;
...
void HtmlFormatter::HandleTagHx(HtmlToken *t)
{
    if (t->IsEndTag()) {
        ...
        if (currY > 0)
            currY += fontSize / 2;
        SetFontBasedOn(CurrFont(), fontSize);
...
void DrawHtmlPage(CanvasDrawingSession^ g, ITextRender *textDraw, Vec<DrawInstr> *drawInstructions, float offX, float offY, bool showBbox, Windows::UI::Color textColor, bool *abortCookie)
{
   ...
   // Pen linePen(Color(0x5F, 0x4B, 0x32), 2.f);
    ...
    for (DrawInstr& i : *drawInstructions) {
        ...
        if (InstrLine == i.type) {
           ...
           // Windows::Foundation::Numerics::float2  p1(bbox.X, y);
			//Windows::Foundation::Numerics::float2 p2(bbox.X + bbox.Width, y);
            if (showBbox) {
               // status = g->DrawRectangle(&debugPen, bbox);
     
            }
			//g->DrawLine(p1, p2, Windows::UI::Colors::Black);
