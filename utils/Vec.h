...
Vec<char> and Vec<wchar_t> a C-compatible string. Although it's
...
class WStrVec : public Vec<wchar_t *>
{
public:
    WStrVec() : Vec() { }
    ...
    wchar_t *Join(const wchar_t *joint=nullptr) {
        str::Str<wchar_t> tmp(256);
    ...
    int Find(const wchar_t *s, size_t startAt=0) const {
        for (size_t i = startAt; i < len; i++) {
            wchar_t *item = At(i);
            if (str::Eq(s, item))
                return (int)i;
    ...
    int FindI(const wchar_t *s, size_t startAt=0) const {
        for (size_t i = startAt; i < len; i++) {
            wchar_t *item = At(i);
            if (str::EqI(s, item))
                return (int)i;
    ...
    size_t Split(const wchar_t *s, const wchar_t *separator, bool collapse=false) {
        size_t start = len;
        const wchar_t *next;
...
private:
    static int cmpNatural(const void *a, const void *b) {
        return str::CmpNatural(*(const wchar_t **)a, *(const wchar_t **)b);
    }
...
class WStrList {
    struct Item {
        ...
        explicit Item(wchar_t *string=nullptr, uint32_t hash=0) : string(string), hash(hash) { }
    };
    ...
    static uint32_t GetQuickHashI(const wchar_t *str) {
...
public:
    ...
    const wchar_t *At(size_t idx) const {
    ...
    int Find(const wchar_t *str, size_t startAt=0) const {
    ...
    int FindI(const wchar_t *str, size_t startAt=0) const {
        uint32_t hash = GetQuickHashI(str);
        Item *item = items.LendData();
        for (size_t i = startAt; i < count; i++) {
